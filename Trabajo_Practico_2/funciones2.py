...
def metodo_de_la_potencia(A, v_inicial=None, iteraciones=100, tol=1e-9):
    """
    Método de la potencia para calcular el autovalor dominante de una matriz.
    
    Parámetros
    ----------
    A : numpy.ndarray
        Matriz cuadrada para la cual se calcula el autovalor de mayor módulo.
    v_inicial : numpy.ndarray, opcional
        Vector inicial para el método. Si no se proporciona, se usa un vector por defecto.
    iteraciones : int, opcional
        Número máximo de iteraciones (por defecto 100).
    tol : float, opcional
        Tolerancia para el criterio de convergencia (por defecto 1e-9).
    
    Returns
    -------
    aval : float
        Autovalor de mayor módulo estimado.
    """

    if v_inicial is None:
        v = v = np.random.rand(A.shape[0])
    else:
        v = v_inicial
    ...
    for _ in range(iteraciones):
        # Multiplicamos por la matriz A
        v_nuevo = A @ v
        
        # Normalizamos el nuevo vector
        v_nuevo = v_nuevo / np.linalg.norm(v_nuevo)
        
        # Verificamos la convergencia (cambio en el vector)
        if np.linalg.norm(v_nuevo - v) < tol:
            break
        
        # Actualizamos el vector
        v = v_nuevo
    
    # Calculamos el autovalor dominante usando Rayleigh quotient
    aval = (v.T @ (A @ v)) / (v.T @ v)
